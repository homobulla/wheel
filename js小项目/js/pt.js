...
window.onload = function () {
    ...
    function GAME(ran) {
                    ...
                    if (oNear) {
                        ...
                        startMove(oNear, aPos[oNear.index], function () {
                            if (finish()) {
                                ...
                                var iRemain = parseInt((oDateNow.getTime() - oDateStart.getTime()) /
                                    1000);
        ...
        function findNearest(obj) {
            ...
            for (i = 0; i < aLi.length; i++) aLi[i] != obj && (isButt(obj, aLi[i]) && (aDistance.push(
                getDistance(obj, aLi[i])), filterLi.push(aLi[i])));

            var minNum = Number.MAX_VALUE;
            var minLi = null;

            for (i = 0; i < aDistance.length; i++) aDistance[i] < minNum && (minNum = aDistance[i], minLi =
                filterLi[i]);
    ...
    function finish() {
        ...
        for (i = 0; i < aLi.length; i++) {
            for (var j = 0; j < aLi.length; j++)
                i == aLi[j]["index"] && aTemp.push(aLi[j].getElementsByTagName("img")[0].src.match(
                    /(\d+)\./)[1])
        }
